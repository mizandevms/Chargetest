				...
				<div id="div1" class="space-y-2" draggable="true" @dragstart="handleDragStart" @dragover="handleDragOver" @drop="handleDrop">
					...
					<div id="bio" draggable="true" @dragstart="handleDragStart" @dragover="handleDragOver" @drop="handleDrop" class="flex flex-col justify-start border border-gray-500 rounded-lg py-1 sm:py-2 px-2 sm:px-4 bg-slate-700">
						<div class="flex justify-between items-baseline">
							...
							<span class="font-light text-xs">({{ bioCount }}/500)</span>
						</div>
					...
					<div id="avatar" draggable="true" @dragstart="handleDragStart" class="flex items-center space-x-6 border border-gray-500 rounded-lg py-2 px-4">
						<div class="shrink-0">
							<img v-if="get_image()" class="h-12 w-12 object-cover rounded-full" :src="get_image()" alt="Current profile photo" />
							<img v-else class="h-12 w-12 object-cover rounded-full" src="https://images.unsplash.com/photo-1580489944761-15a19d654956?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1361&q=80" alt="Current profile photo" />
						</div>
						<label class="block">
							<span class="sr-only">Choose profile photo</span>
							<input accept="image/png, image/jpeg ,image/jpg" @input="form.avatar = $event.target.files[0]" @change="upload_image" type="file" class="block text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-gray-50 file:text-gray-500 hover:file:bg-gray-100 outline-none focus:outline-none h-full w-full" />
						</label>
					</div>
...
export default {
	setup() {
		...
		function submit() {
			...
			const dobReg = /^(?:(?:1[6-9]|[2-9]\d)?\d{2})(?:(?:(\/|-|\.)(?:0?[13578]|1[02])\1(?:31))|(?:(\/|-|\.)(?:0?[13-9]|1[0-2])\2(?:29|30)))$|^(?:(?:(?:1[6-9]|[2-9]\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00)))(\/|-|\.)0?2\3(?:29)$|^(?:(?:1[6-9]|[2-9]\d)?\d{2})(\/|-|\.)(?:(?:0?[1-9])|(?:1[0-2]))\4(?:0?[1-9]|1\d|2[0-8])$/gm
